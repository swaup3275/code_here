...
<body>
	<?php
		...
		if(isset($_SESSION['uid']))
		{
			...
			$query = "SELECT * FROM user ORDER BY solved DESC, attempted DESC";
			$result = mysqli_query($dbc,$query)
				or die('Error querying database.');
						echo '<table>';
						echo '<tr>';
						echo '<th><u>RANK</u>&nbsp&nbsp&nbsp</th>';
						echo '<th><u>USER NAME</u>&nbsp&nbsp&nbsp</th>';
						echo '<th><u>SOLVED</u>&nbsp&nbsp&nbsp</th>';
			...
			while($row=mysqli_fetch_array($result))
			{
					...
					if($attempted!=0)
					{
						$accuracy=(float)$solved/(float)$attempted;
						$accuracy*=100;
					}
					echo '<tr>';
						echo '<td>'.$rank.'</td>';
